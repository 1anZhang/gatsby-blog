...
const Bezier = ({
  ...
  let handleWidth1 = 4, handleWidth2 = 4;
  let handleRadius1 = 6,handleRadius2 = 6;
  let ctx;
  ...
  const handleFlag = (pos) => {
    let flag;
    let dis1 = (pos.x - x1 * cWidth) * (pos.x - x1 * cWidth) +  (pos.y - cHeight + y1 * cHeight) * (pos.y - cHeight + y1 * cHeight);
    let dis2 = (pos.x - x2 * cWidth) * (pos.x - x2 * cWidth) +  (pos.y - cHeight + y2 * cHeight) * (pos.y - cHeight + y2 * cHeight);
    if (dis1 < 40) {
      flag = 1;
    } 
    if (dis2 < 40) {
  ...
  const handleMouseDown = e => {
    ...
    const handleIndex = handleFlag(mousePos);
  ...
  const handleMouseMove = e => {
    ...
    if (drag) {
      ...
      // onAnchorChange(mx, 1- my, handleIndex);
    }
    if (handleIndex) {
      hover = true;
      if (handleIndex === 1) {
